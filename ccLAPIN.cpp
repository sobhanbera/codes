...
|*| @created on : 25-08-2020 07:38:15 PM
\*/
...
void test_cases() {
    string s;
	cin >> s;
	if(lapin(s.substr(0, s.length() / 2), s.substr((s.length() / 2) + (s.length() & 1 ? 1 : 0), s.length()))) {
		cout << "YES" << '\n';
	}else {
		cout << "NO" << '\n';
	}
