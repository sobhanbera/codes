...
|*| @created on : 16-12-2020 07:56:55
\*/
...
double addvalues(double n, double sum, double val) {
	cout << n << ' ' << sum << ' ' << val << endl;
	if(sum == n) {
		return val;
	}else if(sum < n) {
		val += addvalues(n, sum + 2, val + p1);
		val += addvalues(n, sum + 3, val + p2);
	}
	return 0;
}
