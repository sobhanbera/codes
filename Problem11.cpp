...
const int grid[20][20] = {{8,  02, 22, 97, 38, 15, 00, 40, 00, 75,
...
void solution(int _time) {
  ...
  FOR(i, 0, 16) {
    FOR(j, 0, 16) {
      long long curr = grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] *
                       grid[i + 3][j + 3];
      maxans = max(maxans, curr);
      dbg(i, j, i + 1, j + 1, i + 2, j + 2, i + 3, j + 3);
    }
  }

  // diagonally from right-bottom to left-top
  REV(i, 19, 3) {
    REV(j, 19, 3) {
      long long curr = grid[i][j] * grid[i - 1][j - 1] * grid[i - 2][j - 2] *
                       grid[i - 3][j - 3];
